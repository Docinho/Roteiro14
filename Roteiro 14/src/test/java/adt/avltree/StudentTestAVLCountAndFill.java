...
public class StudentTestAVLCountAndFill {
	...
	public void fillWithoutRebalance2() {
		AVLCountAndFillImpl<Integer> avl = new AVLCountAndFillImpl<Integer>();
		...
		for (int j = 0; j < 100; j++) {
			...
			assertEquals(0, avl.LLcount());
			assertEquals(0, avl.LRcount());
			assertEquals(0, avl.RLcount());
			assertEquals(0, avl.RRcount());

			
		}
		
		avl = new AVLCountAndFillImpl<Integer>();
		
		avl.fillWithoutRebalance(new Integer[]{7, 4, 1, 22});
